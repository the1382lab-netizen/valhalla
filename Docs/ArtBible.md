...
**Other masters (Wave 1).** `M_ValhallaGroundBlend` — two world-aligned
layers blended by vertex colour R (`TextureWorldSizeA/B`, `TintA/B`,
`BlendSharpness`, `BlendNoise`). `M_ValhallaFoliage` — masked, two-sided,
Two Sided Foliage; BaseColor alpha is the mask; `TintA/B` picked per tree,
`EdgeFade` 1 on cards / 0 on opaque cores, `RoughnessScale` ~1.6 (leaf
roughness from the scans is too glossy from above). `M_ValhallaWater` —
`NormalMap`, `RippleSize`, `RippleSpeed`, `NormalStrength`, `DeepColor`, `EdgeColor`.
Leaf and ripple textures that no CC0 library has ready are composited or
generated by script (`wave1_leaf_textures.py`, `wave1_water_normal.py`).
