...
| Unreal import | `Valhalla2/Saved/ClaudeOps/eld_import_ue.py` (valhalla_tools.import_kit.reimport_meshes: Nanite on, MI_ slots bound by name, Interchange copies deleted), then `eld_collision_fix2.py` (single convex hull for props and boulders, because Interchange gives these glTFs no simple collision) and `apply_vision_blocker_profile`. `consolidate_materials` is deliberately not run: it would re-point the MI_ slots at M_ValhallaToon copies. |
| New materials | `MI_CliffRock` (Poly Haven `rock_face_03`, CC0, texture set `CliffRock`, recorded in `Import/Textures/texture_sets.json`). After review A3 it is **BaseColor (0.63, 0.91, 1.31), MacroVariation 0.1**. That tint takes the scan's tan-orange mean albedo (linear 0.231 / 0.144 / 0.083) to 75% of the art bible stone `#7A746A` (linear 0.146 / 0.131 / 0.108), whose luminance of 0.133 sits below the grass (`#5E7A3A`, 0.166). Every cliff face, the corner, both clefts, the outcrop and both boulders share this one instance. The other new instances are `MI_CharredTimber` (the Timber set tinted to char), `MI_Shroud` (flat black), `MI_BindGlow` (green-white emissive), `MI_Embers` (orange emissive). All are M_ValhallaPBR instances in `/Game/Valhalla/Materials/Sets`. |
| Review | Blender Workbench renders at the game camera (15 m, 35 deg FOV, 45 deg pitch) in `Valhalla2/Saved/ArtReview/eldmoor/`. No separate Unreal review level: the level agent places the meshes in the real zone. |

Triangle counts are from Blender. Unreal's counts after import are 5-15% lower, because it welds duplicate vertices.
...
| Asset | Footprint X x Y x Z (cm) | Pivot / notes | Collision | Tris (Blender) | Serves |
|---|---|---|---|---|---|
...
| `SM_CliffCleft_Mid` | 256 x 128 x 400 | Added after design review A2. It is the cleft in a **4 m** module, flush with `VB_CliffFace_Mid`. The opening, `MI_Shroud` back wall, `MI_CaveFloor` floor and collision are the same as `SM_CliffCleft`: 150 cm wide, a ceiling at least 186 cm within 30 cm of the centreline, the shroud at local Y -45. Only the crack above is shorter (to 310 cm). Use this, not a Z-scaled `SM_CliffCleft`, which would drop the opening to 128 cm. | BlockAll, per-triangle (walk in) | 1,924 | The Greyfell cleft on the tor step: (384, 1600) cm, z 300, yaw 0; trigger at (384, 1610), z 360 (review A2) |
| `VB_BoulderLarge_A` | 237 x 162 x 220 | A split granite block leaning on two smaller ones. Mossy tops. | VisionBlocker, convex hull | 480 | The cleft screen boulders (3,30) and (9,31) |
| `VB_BoulderLarge_B` | 179 x 118 x 160 | A lower, longer block | VisionBlocker, convex hull | 400 | Hask's line-of-sight rock at (31,42), shelf dressing, scree |
| `VB_RockOutcrop` | 237 x 170 x 142 | A flat top at about 125 cm that an NPC can stand on | VisionBlocker, convex hull | 480 | The crossbowman's rock at Hask's Hold (25,34) |
...
| Asset | Footprint (cm) | Pivot / notes | Collision | Tris (Blender) | Serves |
|---|---|---|---|---|---|
...
| Asset | Footprint (cm) | Pivot / notes | Collision | Tris (Blender) | Serves |
|---|---|---|---|---|---|
| `SM_Forge` | 207 x 89 x 236 | A stone hearth with glowing coals and a small flame. The hood and chimney are at the back (-Y), the bellows on the west end and the quench tub on the east end. Light from `fire_lights` (40 cd, 500 cm). | BlockAll, convex hull | 890 | Harrow's Rest smithy lean-to (x 38-45, y 114-120), with the open side south. The keep's smithy corner at the east end of the outer bailey. |
| `SM_Anvil` | 61 x 45 x 64 | An anvil on a log block, with a hammer on the face | BlockAll, convex hull | 310 | Both smithies, next to the forge |
| `SM_HayBale` | 63 x 42 x 37 | Stacks and scatters | BlockAll, box | 68 | The stables corner (west end of the outer bailey), farmyards |
| `SM_StrawPile` | 141 x 127 x 50 | A loose heap with a half-buried bale | BlockAll, convex hull | 468 | The same |
| `SM_TrainingPost` | 75 x 64 x 154 | A pell: a post on crossed feet with a sackcloth torso and an arm | BlockAll, convex hull | 332 | A2 inner courtyard sparring spots (66,16), (68,18), (78,16). It replaces the stump fallback. |
...
| `SM_ScorchedPost` | 44 x 20 x 172 | A lone charred post with a snapped brace. The pivot is at the post, and the brace reaches to local +X. | BlockAll, convex hull | 72 | Corners and gaps of the burnt houses, fence lines |
| `SM_ScorchedBeams` | 199 x 162 x 49 | Fallen charred beams and boards on an ash bed | BlockAll, convex hull | 228 | The collapsed insides of the burnt houses |
