...
import { CLASS_TEMPLATES, CLASS_COLORS, ClassId } from '@valhalla/shared';
import { AuthClient, type CharacterSummary } from '../systems/AuthClient.js';
...
export class CharacterSelectScene extends Phaser.Scene {
  ...
  private createCharacterCard(
    ...
    const classColor = CLASS_COLORS[char.classId as ClassId] ?? 0xcccccc;
    const hexColor = '#' + classColor.toString(16).padStart(6, '0');
    const template = CLASS_TEMPLATES[char.classId as ClassId];
  ...
  private createNewCharacterButton(cx: number, y: number, w: number): void {
    const h = 48;
