...
export class AuthClient {
   ...
   */
  static async getCharacters(): Promise<CharacterSummary[]> {
   ...
   */
  static async createCharacter(name: string, classId: string): Promise<CharacterSummary> {
    const token = this.getToken();
    if (!token) throw new Error('Not logged in.');
   ...
   */
  static async deleteCharacter(characterId: number): Promise<void> {
    const token = this.getToken();
    if (!token) throw new Error('Not logged in.');
