...
import {
  PAPERDOLL_ANIMS, PAPERDOLL_DIRECTIONS, PAPERDOLL_MANIFEST_URL, PAPERDOLL_ASSET_BASE,
  paperdollAnimKey, paperdollTextureKey, paperdollFrame,
  DEFAULT_BODY_ID,
} from '@valhalla/shared';
import type {
  PaperdollManifest, PaperdollAnim, PaperdollDir, PaperdollLayerEntry,
} from '@valhalla/shared';
...
export class PaperdollRegistry {
  private static _instance: PaperdollRegistry | null = null;

  private manifest: PaperdollManifest | null = null;
  private itemsById = new Map<string, PaperdollLayerEntry>();
  private bodiesById = new Map<string, PaperdollLayerEntry>();
  /** direction -> slot -> index in the composite order */
  private orderIndex = new Map<string, Map<string, number>>();

  static get instance(): PaperdollRegistry {
    if (!PaperdollRegistry._instance) PaperdollRegistry._instance = new PaperdollRegistry();
    return PaperdollRegistry._instance;
  }

  /** True once the manifest has been parsed. Everything no-ops until then. */
  get ready(): boolean { return this.manifest !== null; }
  get cell(): number { return this.manifest?.cell ?? 64; }
  get originY(): number { return this.manifest?.originY ?? 0.9167; }
  get originX(): number { return this.manifest?.originX ?? 0.5; }
  get all(): PaperdollManifest | null { return this.manifest; }

  getItem(layerId: string): PaperdollLayerEntry | undefined { return this.itemsById.get(layerId); }
  hasBody(bodyId: string): boolean { return this.bodiesById.has(bodyId); }
  ...
  resolveBody(bodyId?: string | null): string | null {
    ...
    if (bodyId && this.bodiesById.has(bodyId)) return bodyId;
    if (this.bodiesById.has(DEFAULT_BODY_ID)) return DEFAULT_BODY_ID;
  ...
  load(scene: Phaser.Scene, neededItemIds: Set<string>): Promise<void> {
    return new Promise<void>((resolve) => {
      ...
      scene.load.once('complete', () => {
        const manifest = scene.cache.json.get('paperdoll_manifest') as PaperdollManifest | undefined;
        if (!manifest) {
        ...
        for (const b of manifest.bodies) this.bodiesById.set(b.id, b);
        for (const i of manifest.items) this.itemsById.set(i.id, i);
  ...
  private loadSheets(scene: Phaser.Scene, neededItemIds: Set<string>): Promise<void> {
    const m = this.manifest!;
    const wanted: PaperdollLayerEntry[] = [
      ...m.bodies,
  ...
  private registerAnims(scene: Phaser.Scene, entries: PaperdollLayerEntry[]): void {
    for (const entry of entries) {
      const texture = paperdollTextureKey(entry.id);
      if (!scene.textures.exists(texture)) continue;
      for (const animName of Object.keys(PAPERDOLL_ANIMS) as PaperdollAnim[]) {
...
const DIR_SET = new Set<string>(PAPERDOLL_DIRECTIONS as readonly string[]);
const ANIM_SET = new Set<string>(Object.keys(PAPERDOLL_ANIMS));

export interface ParsedPaperdollKey { layerId: string; anim: PaperdollAnim; dir: PaperdollDir; }

export function parsePaperdollAnimKey(key: string): ParsedPaperdollKey | null {
  if (!key.startsWith('pd_')) return null;
  const parts = key.split('_');
  if (parts.length < 4) return null;
  const dir = parts[parts.length - 1];
  const anim = parts[parts.length - 2];
  if (!DIR_SET.has(dir) || !ANIM_SET.has(anim)) return null;
  return {
    layerId: parts.slice(1, parts.length - 2).join('_'),
    anim: anim as PaperdollAnim,
    dir: dir as PaperdollDir,
  };
}
