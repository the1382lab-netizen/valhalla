...
type SpawnType = 'npc_spawn' | 'enemy_spawn' | 'player_spawn' | 'portal';
...
type PlaceTool = 'select' | 'npc' | 'enemy' | 'player' | 'portal';
...
export const MapEditor: React.FC = () => {
  ...
  const renderCanvas = useCallback(() => {
    ...
    const tw = mapData.tilewidth * zoom;
    const th = mapData.tileheight * zoom;
    ...
    spawnPoints.forEach(sp => {
      const px = sp.x * zoom + pan.x;
      const py = sp.y * zoom + pan.y;
      ...
      const label = sp.label.length > 10 ? sp.label.substring(0, 9) + '…' : sp.label;
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (activeTool === 'npc') {
      ...
      // portal
      newSpawn = { id, type: 'portal', x: wx, y: wy, label: portalTargetZone || 'Portal', templateId: portalTargetZone, width: 64, height: 64 };
    }

    setSpawnPoints(prev => [...prev, newSpawn]);
    setSelectedId(id);
    setIsDirty(true);
  }, [mapData, activeTool, selectedNpcId, selectedEnemyId, portalTargetZone, npcList, findSpawnAt, pan, zoom, worldPos]);
  ...
  }, [isPanning, isDraggingSpawn, selectedId, worldPos]);
  ...
  const cursorStyle = isPanning || isDraggingSpawn
    ? 'grabbing'
    : activeTool === 'select'
      ? 'default'
      : 'crosshair';
        ...
        <div style={{
          ...
          <div style={{ padding: '10px 10px 6px', borderBottom: '1px solid var(--border-color)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {(['select', 'npc', 'enemy', 'player', 'portal'] as PlaceTool[]).map(tool => (
                <button
                  key={tool}
