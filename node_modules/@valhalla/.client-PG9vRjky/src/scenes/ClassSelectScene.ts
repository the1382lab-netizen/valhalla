...
import { ClassId, ALL_CLASS_IDS, ClassTemplate } from '@valhalla/shared';
...
export class ClassSelectScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    ALL_CLASS_IDS.forEach((classId, i) => {
      const template = ClientDataManager.instance.getClass(classId);
      if (!template) return; // skip unknown classes
      const col = i % cols;
  ...
  private createClassCard(
    ...
    const color = ClientDataManager.instance.getClassColor(classId) ?? 0xcccccc;
   ...
   */
  private showNameInput(classId: ClassId): void {
    ...
    const tmpl = ClientDataManager.instance.getClass(classId);
    title.textContent = `Name Your ${tmpl?.name ?? classId}`;
