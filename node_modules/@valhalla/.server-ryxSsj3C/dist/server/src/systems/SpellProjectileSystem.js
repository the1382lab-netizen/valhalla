...
export class SpellProjectileSystem {
    ...
    detonate(proj, detonateX, detonateY, players, npcs, npcSystem, now, isPartyMember) {
        ...
        players.forEach((player) => {
            ...
            if (dist > radius)
                return;
            const hitEvents = this.applyAoeDamage(player, proj._damage, dist, radius, proj._attackerDex, proj._critChance, proj._critDamage, proj.ownerId, now);
     ...
     */
    applyAoeDamage(target, rawDamage, dist, radius, attackerDex, critChance, critDamage, attackerId, now) {
        const events = [];
...
//# sourceMappingURL=SpellProjectileSystem.js.map