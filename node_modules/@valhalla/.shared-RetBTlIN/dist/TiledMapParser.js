 ...
 */
export function parseTiledMap(json) {
    const map = json;
    ...
    const tileLayers = [];
    let collisionGrid = new Array(width * height).fill(0);
    const spawnPoints = [];
            ...
            for (const obj of layer.objects) {
                ...
                if (objType === 'zone_portal' || objType === 'portal') {
                    zoneConnections.push({
                        ...
                        targetZone: (props['targetZone'] ?? props['target_zone'] ?? ''),
                        targetSpawn: {
...
const VALID_SPAWN_TYPES = new Set([
    'enemy_spawn',
    'npc_spawn',
    'player_spawn',
    'item_spawn',
]);
function isSpawnType(type) {
    return VALID_SPAWN_TYPES.has(type);
}
//# sourceMappingURL=TiledMapParser.js.map