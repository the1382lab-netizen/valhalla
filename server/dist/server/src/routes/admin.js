...
adminRouter.get('/state', async (_req, res) => {
    ...
    const ensureZone = (zoneId) => {
        if (!zones[zoneId])
            zones[zoneId] = { players: [], npcs: [], lootBags: [] };
    };
    // Players
...
adminRouter.post('/spawn-npc', async (req, res) => {
    ...
    const room = await getGameRoom();
    if (!room)
        return res.status(503).json({ error: 'Game server not running' });
    try {
        // Access NPCSystem and DataManager through the room
        const npcSystem = room.npcSystem;
...
adminRouter.post('/drop-item', async (req, res) => {
    ...
    if (!room)
        return res.status(503).json({ error: 'Game server not running' });
    try {
        const lootBagSystem = room.lootBagSystem;
        const state = room.state;
        const bagId = lootBagSystem.spawnBag(zoneId, x, y, [{ itemId, quantity: quantity ?? 1 }], state.lootBags);
...
adminRouter.post('/teleport-player', async (req, res) => {
    ...
    try {
        ...
        if (zoneId !== currentZone) {
            ...
            const mapManager = room.mapManager;
            // Ensure zone is cached
            room.loadZoneCache(zoneId);
