 ...
 */
export function removeItem(player, slotIndex, qty = 1) {
 ...
 */
export function equipItem(player, inventorySlotIndex) {
...
export function swapInventorySlots(player, fromIndex, toIndex) {
    ...
    if (fromIndex < 0 || fromIndex >= player.inventory.length)
        return false;
    if (toIndex < 0 || toIndex >= player.inventory.length)
        return false;
 ...
 */
export function unequipItemToSlot(player, slotType, targetIndex) {
    const currentlyEquipped = getEquipField(player, slotType);
    if (!currentlyEquipped)
        return false;
