...
import { LOOT_BAG_MERGE_RANGE, LOOT_BAG_PICKUP_RANGE, LOOT_BAG_DESPAWN_MS, LOOT_BAG_MAX_SLOTS, } from '@valhalla/shared';
export class LootBagSystem {
     ...
     * Returns the bag ID.
     */
    spawnBag(zoneId, x, y, items, lootBags) {
        if (items.length === 0)
            return '';
     ...
     */
    lootItem(player, bag, slotIndex, quantity) {
        if (!this.canPlayerReachBag(player, bag))
            return false;
        if (slotIndex < 0 || slotIndex >= bag.items.length)
            return false;
        ...
        if (!addItem(player, bagSlot.itemId, actualQty)) {
    ...
    lootAll(player, bag) {
        ...
        for (let i = bag.items.length - 1; i >= 0; i--) {
            ...
            if (addItem(player, bagSlot.itemId, bagSlot.quantity)) {
     ...
     */
    canPlayerReachBag(player, bag) {
        if (player.zoneId !== bag.zoneId)
            return false;
...
//# sourceMappingURL=LootBagSystem.js.map