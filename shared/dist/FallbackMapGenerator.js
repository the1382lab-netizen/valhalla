 ...
 */
export function generateFallbackMap(width = 64, height = 64, tileSize = 64) {
    ...
    // Town gates (openings)
    set(wallsData, 10, 18, 0);
    collisionGrid[18 * width + 10] = 0;
    set(wallsData, 11, 18, 0);
    collisionGrid[18 * width + 11] = 0;
    set(wallsData, 18, 10, 0);
    collisionGrid[10 * width + 18] = 0;
    set(wallsData, 18, 11, 0);
    collisionGrid[11 * width + 18] = 0;
    // ── A building inside town ──────────────────────────────
    for (let x = 5; x <= 9; x++) {
        set(wallsData, x, 5, TILE.STONE_WALL);
        setCollision(x, 5);
        set(wallsData, x, 8, TILE.STONE_WALL);
        setCollision(x, 8);
    }
    for (let y = 5; y <= 8; y++) {
        set(wallsData, 5, y, TILE.STONE_WALL);
        setCollision(5, y);
        set(wallsData, 9, y, TILE.STONE_WALL);
        setCollision(9, y);
    }
    // Door
    set(wallsData, 7, 8, 0);
    collisionGrid[8 * width + 7] = 0;
    ...
    set(wallsData, 28, height - 5, TILE.STONE_WALL);
    setCollision(28, height - 5);
    set(wallsData, 34, height - 5, TILE.STONE_WALL);
    setCollision(34, height - 5);
    for (let x = 28; x <= 34; x++) {
        set(wallsData, x, height - 5, TILE.STONE_WALL);
        setCollision(x, height - 5);
    }
    // Opening in the wall for cave entrance
    set(wallsData, 31, height - 5, 0);
    collisionGrid[(height - 5) * width + 31] = 0;
    set(wallsData, 30, height - 5, 0);
    collisionGrid[(height - 5) * width + 30] = 0;
...
//# sourceMappingURL=FallbackMapGenerator.js.map